...
RSpec.describe ActiveSecurity::Privileged do
  before do
  ...
  context "when only plugin configured" do
    ...
    context "with only privileged scope" do
      ...
      it "does log" do
        output = capture(:stderr) { query_privileged_scope }
        expect(output).to include("[Privileged] (ActiveRecord::Relation) does not have secure scope")
      end

      context "when not found" do
        subject(:query_privileged_scope) do
          ar_with_active_security.privileged.find(2)
        end
  ...
  context "when finders also configured" do
    ...
    context "with no scope" do
      ...
      it "does log" do
        output = capture(:stderr) { query_no_scope }
        expect(output).to match(/\[Privileged\] \(\w*\) does not have secure scope/)
      end
